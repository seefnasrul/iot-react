/*toaster*/
export const SET_TOAST_NOTIFICATION  = "SET_TOAST_NOTIFICATION";

/*auth*/
export const LOGIN = 'LOGIN';
export const REGISTER = 'REGISTER';
export const SET_TOKEN_DATA = 'SET_TOKEN_DATA';
export const SET_USER_DATA = 'SET_USER_DATA';
export const PROFILE_UPDATE = 'PROFILE_UPDATE';
export const UNSET_AUTH = 'UNSET_AUTH';

/*home*/
export const GET_HOME_DATA = 'GET_HOME_DATA';
export const SET_HOME_DATA = 'SET_HOME_DATA';


/*exam*/
export const GET_EXAM_DATA = 'GET_EXAM_DATA';
export const SET_EXAM_DATA = 'SET_EXAM_DATA';
export const SET_EXAM_ANSWER_TO_SERVER = 'SET_EXAM_ANSWER_TO_SERVER';
export const SET_EXAM_ANSWER = 'SET_EXAM_ANSWER';


/*device*/
export const CREATE_NEW_DEVICE = "CREATE_NEW_DEVICE";
export const GET_DEVICE_BY_ID = "GET_DEVICE_BY_ID";
export const UPDATE_DEVICE_BY_ID = "UPDATE_DEVICE_BY_ID";


/*dahsboard*/
export const SAVE_DASHBOARD = "SAVE_DASHBOARD";
export const SET_DASHBOARD_DATA = "SET_DASHBOARD_DATA";
export const GET_DASHBOARD_DATA = "GET_DASHBOARD_DATA";
export const SET_DASHBOARD_TIMERANGE = "SET_DASHBOARD_TIMERANGE";
export const SAVE_DASHBOARD_TIMERANGE = "SAVE_DASHBOARD_TIMERANGE";